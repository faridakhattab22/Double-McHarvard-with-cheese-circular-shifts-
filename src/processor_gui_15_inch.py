...
import subprocess, threading, queue, re, os, random, time

try:
    from ctypes import windll
    windll.shcore.SetProcessDpiAwareness(1) # For Windows 8.1 / 10 / 11
except:
    try:
        import ctypes
        ctypes.windll.user32.SetProcessDPIAware() # Older Windows fallback
    except:
        pass # If you are on Mac/Linux, this skips without crashing
...
class ProcessorGUI:
    ...
    def _start(self):
        ...
        # FIX: Use safe, native Python timing instead of system Tk calls
        self._start_time = int(time.perf_counter() * 1000) 
        
        try:
            # FIX: Ensure paths with spaces or weird system variables don't crash Popen
            clean_path = os.path.normpath(BINARY_NAME)
            self.process = subprocess.Popen(
                [clean_path],
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                stdin=subprocess.PIPE, text=True, bufsize=1, encoding="utf-8")
    ...
    def _show_success_fx(self):
        ...
        # FIX: Force absolute opaque background to prevent underneath UI bleeding
        overlay = tk.Canvas(self.root, bg=T["BG"], highlightthickness=0, borderwidth=0)
        overlay.place(relx=0, rely=0, relwidth=1, relheight=1)
        
        # Draw background grid lines safely
        for i in range(0, w, 40):
            overlay.create_line(i, 0, i, h, fill=T["PANEL"], width=1)
        for i in range(0, h, 40):
            overlay.create_line(0, i, w, i, fill=T["PANEL"], width=1)
        ...
        # FIX: Switched to crisp standard system fonts to completely eliminate haziness
        overlay.create_text(w//2 + 3, h//2 - 57, text=txt, 
                            fill=T["ACCENT2"], font=("Arial", 42, "bold"))
        main_txt = overlay.create_text(w//2, h//2 - 60, text=txt, 
                                       fill=T["GREEN"], font=("Arial", 42, "bold"))
        
        stats = f"TOTAL CYCLES: {self.clock_cycle}   •   PC REACHED: {self.pc}   •   SREG: {self.sreg:08b}"
        overlay.create_text(w//2, h//2 + 20, text=stats, 
                            fill=T["FG"], font=("Consolas", 14, "bold"))

        # Floating Binary Particles (Reduced count slightly to completely eliminate GPU lag)
        particles = []
        for _ in range(45): 
            px, py = random.randint(0, w), random.randint(0, h)
            char = random.choice(["0", "1", "OK"])
            clr = random.choice([T["GREEN"], T["ACCENT"], T["PURPLE"], T["MUTED"]])
            p = overlay.create_text(px, py, text=char, fill=clr, 
                                    font=("Consolas", random.randint(10, 14), "bold"))
            particles.append({
                'id': p, 
        ...
        # Dismiss Button Frame
        btn_frame = tk.Frame(overlay, bg=T["GREEN"], padx=2, pady=2)
        overlay.create_window(w//2, h - 150, window=btn_frame)
        
        btn = tk.Button(btn_frame, text=" DISMISS NOTIFICATION ", 
                        bg=T["BG"], fg=T["GREEN"], font=("Arial", 11, "bold"),
                        relief="flat", padx=30, pady=12, command=overlay.destroy,
                        activebackground=T["GREEN"], activeforeground=T["BG"],
                        cursor="hand2")
        btn.pack()
        ...
        def anim(step=0):
            ...
            # Pulse main text color cleanly
            if step % 20 == 0:
                curr = overlay.itemcget(main_txt, "fill")
                next_c = T["FG"] if curr == T["GREEN"] else T["GREEN"]
                overlay.itemconfig(main_txt, fill=next_c)

            # Smoothly step animation updates
            for p in particles:
                overlay.move(p['id'], p['vx'], p['vy'])
                pos = overlay.coords(p['id'])
                if len(pos) >= 2 and pos[1] > h:
                    overlay.coords(p['id'], random.randint(0, w), -20)
            ...
            # FIX: Adjusted to 35ms to sync perfectly with laptop display refresh bounds
            self.root.after(35, lambda: anim(step + 1))
